...
- Local 기본/다른 관점 user prompt 끝에는 설명 문장을 한국어로 제한하고 ROS2 식별자·field·code·log 원문은
  보존하는 Local 전용 출력 규칙을 추가한다. 공통 SYSTEM instruction과 Cloud prompt에는 이 규칙을 추가하지 않는다.
