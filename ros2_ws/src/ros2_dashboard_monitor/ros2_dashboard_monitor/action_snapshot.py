        ...
        if client_created:
            ...
            for part, state in (action.get('qos') or {}).items():
                ...
                if isinstance(state, dict):
                    ...
                    local_status = local_state.get('qos_status')
                    observed_status = state.get('qos_status')
                    if (
                        part in {'goal', 'result', 'cancel'}
                        and local_status in {'compatible', 'partial', 'incompatible'}
                    ) or (
                        part in {'feedback', 'status'}
                        and observed_status in {'unknown', 'observed'}
                        and local_status == 'compatible'
                    ):
                        state.update({
