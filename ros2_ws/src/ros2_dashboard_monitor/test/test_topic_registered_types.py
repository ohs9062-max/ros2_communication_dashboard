...
from ros2_dashboard_monitor.ros2_topic.filters import is_supported_type
...
def test_registered_importable_messages_extend_monitor_supported_types(
    ...
    monkeypatch.setenv('INTERFACE_REGISTRY_PATH', str(config_dir / 'interface_registry.yaml'))
    monkeypatch.setenv('INTERFACE_PACKAGES_REGISTRY_PATH', str(config_dir / 'interface_packages.yaml'))
...
def test_topic_alert_policy_names_are_loaded_from_monitor_config() -> None:
    config = _monitor_config({
        'topics': {
            'required_stream_names': ['/imu', '/scan'],
            'command_names': ['/cmd_vel', '/robot/command'],
        },
    })

    assert config.topics_required_stream_names == ('/imu', '/scan')
    assert config.topics_command_names == ('/cmd_vel', '/robot/command')


def test_topic_alert_policy_names_default_to_empty_tuples() -> None:
    config = _monitor_config({})

    assert config.topics_required_stream_names == ()
    assert config.topics_command_names == ()
...
def test_command_topic_is_excluded_from_missing_and_stale_alerts() -> None:
    topic = {
        'name': '/robot/command',
        'publisher_count': 1,
    ...
    alerts = build_alerts(
        ...
        detected_at=10.0,
        stale_timeout_sec=3.0,
        command_names=('/robot/command',),
    )

    assert alerts == []


def test_required_stream_topic_reports_missing_alert() -> None:
    topic = {
        'name': '/robot/state',
        'publisher_count': 1,
    ...
    alerts = build_alerts(
        ...
        detected_at=4.0,
        stale_timeout_sec=3.0,
        required_stream_names=('/robot/state',),
    )
...
def test_monitor_only_topic_subscription_is_removed_but_state_is_retained(
    ...
    monkeypatch.setattr(
        'ros2_dashboard_monitor.ros2_topic.runtime.'
