...
def test_default_apply_paths_stay_ros_workspace_relative(monkeypatch):
    ...
    assert default_apply_status_path().name == 'interface_apply_status.yaml'
...
def test_duplicate_workspace_packages_reports_selected_package(tmp_path: Path):
    workspace = tmp_path / 'backend'
    first = workspace / 'src' / 'uploaded_interfaces' / 'packages' / 'rths_interfaces'
