...
from ros2_dashboard_monitor.interface_lab.management import manual_interfaces
from ros2_dashboard_monitor.interface_lab.management import registry as interface_registry
from ros2_dashboard_monitor.interface_lab.management.registry import InterfaceUploadError
...
def test_manual_definition_creates_uploaded_interfaces_package(tmp_path, monkeypatch):
    registry_path = tmp_path / 'config' / 'interface_registry.yaml'
    monkeypatch.setenv('INTERFACE_REGISTRY_PATH', str(registry_path))
    monkeypatch.setattr(manual_interfaces, 'generated_interface_package_root', lambda: tmp_path / 'src' / 'uploaded_interfaces')
