...
def test_default_registry_paths_stay_ros_workspace_relative(monkeypatch):
    ...
    assert workspace.name == 'ros2_ws'
    assert default_registry_path().name == 'interface_registry.yaml'
    assert default_registry_path().parent.name == 'config'
    assert default_registry_path().parent.parent.name == 'ros2_dashboard_monitor'
    assert package_name == 'uploaded_interfaces'
    assert package_path == (workspace / 'src' / 'uploaded_interfaces' / 'generated_interfaces').resolve()
