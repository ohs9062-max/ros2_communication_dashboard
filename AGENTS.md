...
AGENTS.md는 이 프로젝트의 작업 기준으로 적용한다.
같은 세션에서 AGENTS.md를 명시적으로 반복 read하지 않는다.

### 새 세션 / 최초 작업

새 세션의 최초 작업에서는 다음을 기준본으로 1회 확인한다.

1. `.codex/CURRENT_STATUS.md` 전체
2. `.codex/WORK_LOG.md`의 최신 작업 3개

이 최초 확인은 생략하지 않는다.

`WORK_LOG.md` 파일 자체에는 최근 20~30개 작업을 유지한다.
이는 보관 범위이며, 최초 작업에서 읽는 범위는 최신 3개다.


### 같은 세션의 후속 작업

이미 기준본을 확인했다면 매 작업마다 문서를 다시 전체 읽지 않는다.

- `CURRENT_STATUS.md` 전체 재읽기 금지
- 이미 확인한 `WORK_LOG` 작업 block 재읽기 금지
- archive 전체 읽기 금지

문서가 변경되지 않았다면 추가 read하지 않는다.

문서가 변경됐다면 전체 문서를 다시 읽지 말고,
마지막으로 확인한 상태 이후의 `git diff` 또는
변경된 section만 확인한다.


### 추가 컨텍스트가 필요한 경우

최신 WORK_LOG 3개만으로 현재 요청의 맥락이 부족하면:
...
로 `WORK_LOG.md`를 검색한다.

검색된 관련 작업 block만 추가로 읽는다.

그래도 필요한 과거 근거가 없을 때만
`.codex/archive/`를 검색하고 관련 block만 읽는다.

WORK_LOG와 archive 전체 읽기는 기본적으로 하지 않는다.


### CURRENT_STATUS 재확인 조건

다음 경우에는 `CURRENT_STATUS.md`의 관련 section만 다시 확인한다.

- 프로젝트 구조 또는 책임 경계를 변경하는 작업
- Alert / QoS / DB / Multi-domain / Interface Lab / 설치 정책 변경
- 현재 제한 또는 미검증 범위를 판단해야 하는 작업
- 코드와 기존 기억이 충돌하는 경우

이 경우에도 전체 파일 재읽기보다
관련 section과 변경 diff 확인을 우선한다.


### 작업 종료

작업 종료 시 `.codex/WORK_LOG.md`에
날짜, 변경 핵심, 중요한 제약, 검증 결과만 짧게 기록한다.

구조, 정책, 검증 기준, 다음 작업이 변경된 경우에만
`.codex/CURRENT_STATUS.md`의 현재 사실을 갱신한다.

WORK_LOG가 30개를 넘으면 가장 오래된 항목을 날짜 단위로
`.codex/archive/WORK_LOG_YYYY-MM-DD.md`에 원문 그대로 이동한다.

구현하지 않았거나 검증하지 않은 내용을 완료로 기록하지 않는다.
문서와 코드가 다르면 실제 코드와 실행 결과를 우선한다.
## 2. 프로젝트 목적과 기술 스택
