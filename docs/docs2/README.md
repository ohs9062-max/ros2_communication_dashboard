...
```text
`service_call_runtime.py` 함수 전체 L84-L157
= `service_call_runtime.py`의 `call_service()` 함수 시작부터 끝
```

라인 번호는 2026-09-01 현재 코드 기준이다. 코드가 수정되면 함수 이름을 먼저 검색하고 L 번호를 다시 맞춰야 한다.
...
**Durability** = 늦게 들어온 구독자가 과거 메시지를 받을지 정하는 정책 = Interface Lab Auto는 상대 endpoint와 호환되는 값을 선택하고 Manual은 VOLATILE 또는 TRANSIENT_LOCAL을 지정합니다.

**History** = 메시지를 어떤 방식으로 보관할지 정하는 정책 = Interface Lab Manual은 KEEP_LAST와 KEEP_ALL을 지원하며 Service Auto는 Discovery에서 알 수 없어 local 기본값을 사용합니다.
...
**Interface Lab QoS** = Auto는 Topic Graph 또는 Service Fast DDS Discovery에서 호환 profile을 선택하고, Manual은 기본 4개와 고급 4개 정책을 실제 entity에 적용합니다.

**Service QoS** = Auto는 원격 Request Reader와 Response Writer를 한 Client profile로 함께 만족하도록 발견값을 계산하고 History/Depth만 local Service 기본값을 사용합니다.

**Action QoS** = Goal·Result·Cancel Service는 Fast DDS, Feedback·Status Topic은 Graph를 사용해 5개 profile을 각각 선택합니다.

**Deadline** = 정해진 시간 안에 메시지가 계속 도착해야 하는지 정하는 정책 = Auto 발견값 또는 Manual duration을 실제 profile에 적용합니다.

**Lifespan** = 발행된 메시지가 얼마 동안 유효한지 정하는 정책 = Service Auto는 관찰된 Response Writer 값, Manual은 입력 duration을 적용합니다.

**Liveliness** = 발행자가 살아 있음을 어떤 방식으로 확인할지 정하는 정책 = Auto 발견값 또는 Manual의 Jazzy 지원 enum을 적용합니다.

**TRANSIENT_LOCAL** = 늦게 들어온 구독자에게 이전 메시지를 전달하는 Durability 정책 = Auto 호환 선택과 Manual에서 사용할 수 있습니다.

**KEEP_ALL** = 수신한 메시지를 제한 없이 보관하는 History 정책 = Interface Lab Manual에서 사용할 수 있습니다.
