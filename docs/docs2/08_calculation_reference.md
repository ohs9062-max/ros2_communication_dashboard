...
| Service 외부 Node/Call/QoS | Service snapshot 조립 | `service_snapshot.py assemble_service_snapshot()` L16-L116 |
| Action 외부 Node/Goal/QoS | Action snapshot 조립 | `action_snapshot.py assemble_action_snapshot()` L15-L136 |
...
| 영역 | 계산 | 현재 코드 |
|---|---|---|
| Topic Graph | active/no_subscriber/waiting_publisher/inactive | `ros2_topic/models.py topic_status()` L49-L69 |
| Topic 대표 | Graph status 또는 never_received/stale | `ros2_topic/snapshot.py _effective_status()` L157-L177 |
| Service | unknown/active/waiting_server/inactive | `ros2_service/models.py service_status()` L37-L60 |
| Action | unknown/active/waiting_server/inactive | `ros2_action/models.py action_status()` L63-L83 |
| Node 발견 | active | `ros2_node/models.py node_status()` L27-L41 |
...
Service 목록의 최근 사용자 Call 상태는 `service_snapshot.py` L16-L116에서,
Action의 최근 Goal/Feedback/Result는 `action_snapshot.py` L15-L136에서 Graph 상태와 별도 필드로 병합한다.
...
| age와 stale 상태 | `ros2_topic/hz.py hz_status()` L25-L41 |
| `message_count / window_sec`와 응답 | `ros2_topic/hz.py build_hz_snapshot()` L42-L71 |
| callback timestamp 저장 | `ros2_topic/subscriptions.py update_subscription_entry()` L46-L69 |
...
관찰 Action의 elapsed는 `ros2_action/subscriptions.py _elapsed_time_ms()` L269-L280에서 계산한다.
Service Call 실행과 기록은 `interface_lab/execution/service_call_runtime.py call_service()` L84-L157,
Action Goal 실행과 기록은 `interface_lab/execution/action_goal_runtime.py send_goal()` L84-L124이
...
| Monitor active/resolved 응답 | `ros_monitor.py alerts()` L378-L425 |
...
| Service meta | `ros2_service/models.py service_meta()` L66-L147 |
| Action meta | `ros2_action/models.py action_meta()` L114-L159 |
| Node meta | `ros2_node/models.py node_meta()` L56-L90 |
