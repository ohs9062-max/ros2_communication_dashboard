...
| Service 외부 Node/Call/QoS | Service snapshot 조립 | `service_snapshot.py assemble_service_snapshot()` L16-L114 |
...
| Topic 대표 | Graph status 또는 never_received/stale | `ros2_topic/snapshot.py _effective_status()` L157-L178 |
...
| Node 발견 | active | `ros2_node/models.py node_status()` L27-L40 |
...
Service 목록의 최근 사용자 Call 상태는 `service_snapshot.py` L16-L114에서,
...
| callback timestamp 저장 | `ros2_topic/subscriptions.py update_subscription_entry()` L46-L68 |
...
관찰 Action의 elapsed는 `ros2_action/subscriptions.py _elapsed_time_ms()` L269-L279에서 계산한다.
Service Call 실행과 기록은 `interface_lab/execution/service_call_runtime.py call_service()` L84-L156,
...
| Action 후보 | `ros2_action/alerts.py` L21-L177 |
| Node 후보 | `ros2_node/alerts.py` L13-L44 |
| source 결합과 lifecycle | `alert_assembler.py` |
| Monitor active/resolved 응답 | `ros_monitor.py alerts()` L366-L412 |
