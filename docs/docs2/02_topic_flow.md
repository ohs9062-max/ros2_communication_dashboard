...
| 단계 | 현재 코드 위치 | 역할 |
|---:|---|---|
| 1 | `ros2_topic/runtime.py TopicRuntime.update()` L103-L141 | Graph 수집 결과를 현재 cache와 병합하고 사라진 Topic 처리 |
...
| 4 | `ros2_topic/subscriptions.py update_subscription_entry()` L46-L69 | callback timestamp와 preview cache 갱신 |
...
| 8 | `transport/routers/monitoring.py` L16-L61 | Topic 목록/latest/Hz/image-preview API |
| 9 | `frontend/src/hooks/useTopicDashboard.js` L23-L252 | 목록·Alert·Node·latest·Hz·Camera polling |
| 10 | `frontend/src/pages/TopicsPage.jsx` L19-L185 | 주요/전체, 검색, 상태 필터, 목록·상세 표시 |

## 상태와 Alert

Graph 원본 `status`는 `ros2_topic/models.py topic_status()` L49-L69에서 계산하고, 목록 대표
...
`_topic_alerts()` L61-L140, `_topic_message_alerts()` L141-L194가 담당한다.
...
`interface_lab/execution/topic_runtime.py dashboard_state_by_topic()` L124-L130에서 별도로 합친다.
...
`transport/routers/monitoring.py` L52-L61, Frontend는
