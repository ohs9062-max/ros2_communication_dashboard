...
| 단계 | 현재 코드 위치 | 역할 |
|---:|---|---|
| 1 | `ros2_node/runtime.py NodeRuntime.update()` L74-L166 | Node filter와 여섯 통신 역할 수집, disconnected debounce |
| 2 | `ros2_node/runtime.py _graph_by_node()` L177-L193 | Topic/Service Graph 역할 조회 |
...
`ros2_node/models.py full_node_name()` L18-L24가 namespace와 name을 하나의 정규화된 full name으로 만든다.
...
`topology.py build_role_node_index()` L19-L39와 `related_nodes()` L42-L54는 Node의 여섯 역할을
...
Frontend `utils/nodeFilters.js` L1-L13은 내부 Node와 최종 `is_primary`만 사용한다.
