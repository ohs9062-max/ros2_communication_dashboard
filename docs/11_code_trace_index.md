...
| spin | `ros_monitor.py L665~L675` |
| Graph 주기 갱신 | `ros_monitor.py L677~L683` |
...
Active check 호환 코드는 `service/active_check*.py`에 남아 있지만 `ros_monitor.py L677~L683`에서 실행하지 않는다.
...
```text
RosMonitor.alerts()                       ros_monitor.py L496~L564
