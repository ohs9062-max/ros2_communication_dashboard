...
| spin | `ros_monitor.py L669~L679` |
| Graph 주기 갱신 | `ros_monitor.py L681~L687` |
...
```text
config_loader.py L191~L330
→ topic/runtime.py L125~L231
→ topic/runtime.py L372~L505
→ topic/runtime.py L506~L523
→ topic/preview.py L15~L21
→ topic/runtime.py L75~L105
→ routers/monitoring.py L16~L40
→ rosApi.js L45~L55
→ useTopicDashboard.js L17~L178
...
| 세부 기능 | 코드 위치 |
|---|---|
| registered msg 병합 | `config_loader.py L289~L330` |
| Graph/update/cache | `topic/runtime.py L125~L231` |
| 지원 타입 판정 | `topic/runtime.py L320~L371` |
| Subscription/내부 endpoint 판정 | `topic/runtime.py L372~L467` |
| cleanup | `topic/runtime.py L468~L505` |
| callback/latest 저장 | `topic/runtime.py L506~L523` |
| Hz snapshot | `topic/runtime.py L524~L563`, `topic/hz.py L14~L70` |
...
| missing/stale Alert | `topic/alerts.py L161~L281` |

## 5. Service 끝까지 추적

```text
service/runtime.py L90~L153
...
| 세부 기능 | 코드 위치 |
|---|---|
| Graph와 disconnected | `service/runtime.py L90~L153` |
...
| Graph+Node 관계+Call 상태 병합 | `ros_monitor.py L173~L232`, `L691~L709` |
| Timeout Alert | `service/alerts.py L10~L67` |

Active check 호환 코드는 `service/active_check*.py`에 남아 있지만 `ros_monitor.py L681~L687`에서 실행하지 않는다.
...
| layout/filter | `graphTransform.js L356~L694` |
| Visualization polling | `useVisualizationGraph.js L18~L274` |
| React Flow 화면 | `VisualizationPage.jsx L11~L385` |

## 8. Alert

```text
RosMonitor.alerts()                       ros_monitor.py L500~L568
→ build_alerts()                          topic/alerts.py L27~L57
...
→ retain_alerts()                         topic/alerts.py L60~L127
→ build_alert_meta()                      topic/alerts.py L130~L158
...
- custom msg 마지막 값 `{}`: `topic/runtime.py L506~L523` → `topic/preview.py L15~L21` → `TopicTable.jsx L46~L146`
- Topic Hz/stale: `topic/hz.py L14~L70` → `topic/alerts.py L161~L281`
...
- Alert가 해결 후 남음: `topic/alerts.py L60~L127`의 `retain_alerts()` 확인
- 주요 Node 누락: `/ros/nodes` 관계 → `primaryFilters.js L1~L78` → `nodeFilters.js L1~L77`
