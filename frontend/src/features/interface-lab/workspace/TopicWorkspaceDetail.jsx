  ...
  return (
    <>
      ...
      {(view === 'details' || view === 'advanced') && <>
        ...
        <ConnectionList empty="Graph에서 이 메시지 타입으로 열린 Topic이 없습니다." items={item.connectedTopics} render={(topic) => [`Domain ${topic.domain_id}`, topic.name, firstType(topic.type ?? topic.types) ?? '-', `publishers ${topic.publisher_count ?? 0}`, `subscribers ${topic.subscriber_count ?? 0}`].join(' · ')} />
